<template>
  ...
  <li class="-mb-px mr-1">
    <a class="bg-white inline-block py-2 px-4  font-semibold" :class="{ 'border-l border-t border-r rounded-t text-blue-dark': tabActive=='charts', 'text-blue hover:text-blue-darker': tabActive!='charts'}" href="#" @click="tabActive='charts'">Grafici</a>
  </li>
  <li class="mr-1">
    <a class="bg-white inline-block py-2 px-4  font-semibold" :class="{ 'border-l border-t border-r rounded-t ': tabActive=='csv', 'text-blue hover:text-blue-darker': tabActive!='csv'}" href="#" @click="tabActive='csv'">CSV</a>
  </li>
      </ul>
            <div
        class="mx-2 my-2 flex flex-col  text-white p-2 bg-primary rounded-xl border-2 h-full" v-if="tabActive=='charts'"
      >
      ...
      <div
        class="mx-2 my-2 flex flex-col  text-white p-2 bg-primary rounded-xl border-2 h-full" v-if="tabActive=='csv'"
      >
        <div class="flex flex-col md:flex-row mt-3 justify-stretch lg:flex-col">
          <label for="sub_select">Seleziona una campagna</label>
          <select
        ...
        >
          ...
          v-if="role=='ROLE_COMPANY_ADMIN'||(role=='ROLE_ADMIN'&&adminCompany!=null)||(role=='ROLE_MOBILITY_MANAGER'&& actualCompany!=null)"
        >
          <label for="sub_select">Azienda selezionata: {{adminCompany?adminCompany.item.name:actualCompany.item.name}}</label>
          <!-- <select
            class="focus:border-blue-600 border-2 p-2 mb-2 md:mb-0 lg:mb-2 flex-1 md:mr-2 lg:mr-0 appearance-none text-primary bg-white"
            name="sub_select"
            id="company"
  ...
  methods: {
    ...
    exportCsv(){
      ...
      if (this.role=='ROLE_ADMIN'&&this.adminCompany==null) {
        ...
        this.getCompanyCsv({campaignId:this.selectedCampaign.id,companyId:this.selectedCompany,from:this.selectedMonth?this.selectedMonth.value:null,to:null})
        //get stat for locations
        else this.getLocationCsv({campaignId:this.selectedCampaign.id,companyId:this.selectedCompany,from:this.selectedMonth?this.selectedMonth.value:null,to:null})
      }
